...
public class DevoteeDao {
    ...
    // STEP 3.2: Update fromCursor to read the new columns.
    public Devotee fromCursor(Cursor cursor) {
        ...
        int aadhaarCol = cursor.getColumnIndexOrThrow("aadhaar");
        int panCol = cursor.getColumnIndexOrThrow("pan");
        ...
        return new Devotee(
            ...
            cursor.getString(emailCol), cursor.getString(genderCol),
            cursor.getString(aadhaarCol), cursor.getString(panCol), cursor.getString(extraJsonCol)
        );
    }
    ...
    // STEP 3.3: Update resolveOrCreateDevotee signature and logic.
    public long resolveOrCreateDevotee(String rawName, String rawMobile, String address, Integer age, String email, String gender, String aadhaar, String pan) {
        ...
        if (best == null) { for (Devotee cand : sameMobile) { double s = jaroWinklerSim(normalizeName(rawName), normalizeName(cand.getFullName())); if (s > bestSim) { bestSim = s; best = cand; } } if (bestSim < 0.92) best = null; }
        if (best != null) { if (bestSim < 0.999) { logFuzzyMerge(mobile10, rawName, best.getFullName(), bestSim); } return best.getDevoteeId(); }

        Devotee fresh = new Devotee(null, rawName, nameNorm, mobile10, address, age, email, gender, aadhaar, pan, null);
        return insertAndGetId(fresh);
    }
    
    // STEP 3.4: Update insertAndGetId to handle new fields.
    private long insertAndGetId(Devotee d) {
        ...
        values.put("aadhaar", d.getAadhaar());
        values.put("pan", d.getPan());
