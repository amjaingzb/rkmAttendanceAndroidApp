...
public ActiveBatchData getActiveDonationBatchOrCreateNew() {
    ...
    if (activeBatch == null) {
        long newBatchId = donationBatchDao.insertNewBatch();
...
public void recordDonationInBatch(long devoteeId, double amount, String paymentMethod, String refId, String purpose, String user, long batchId) {
    // Generate a unique receipt number, e.g., RKM-YYYYMMDD-DONATIONID
    String timestamp = DateTimeFormatter.ofPattern("yyyyMMdd").format(LocalDateTime.now());
    // We insert first to get the donation_id, then update with the receipt number.
...
public static class WhatsAppInvite {
    ...
    public WhatsAppInvite(String link, String messageTemplate) { this.link = link; this.messageTemplate = messageTemplate; }
}
